...
class TrainingConfig:
    ...
    # Hiperparâmetros (mesmos do notebook)
    lr_defensor: float = 1e-3
    lr_atacante: float = 2e-4
    adam_betas: tuple = (0.5, 0.999)
    epsilon: float = 0.3          # escala da perturbação
    epochs_pretrain: int = 5      # pré-treino do defensor
    ...
    historico: dict = field(default_factory=lambda: {
        "taxa_evasao": [],
        "acuracia_defensor": [],
        "loss_atacante": [],
        "loss_defensor": [],
...
class AdversarialTrainer:
    ...
    Fluxo de cada rodada:
      ...
      2. Defensor retreina com as amostras perturbadas (aprende a detectá-las)
    ...
    def __init__(self, config: TrainingConfig):
        ...
        self.atacante = Atacante(config.noise_dim, config.input_dim).to(self.device)

        self.criterio = nn.BCELoss()
        self.opt_def = torch.optim.Adam(
            self.defensor.parameters(), lr=config.lr_defensor
        )
    ...
    def carregar_dados(
        ...
        # Loader geral (benigno + DDoS misturados)
        ...
        # Loader apenas DDoS (base para perturbações)
        mask_ddos = y_train == 1
        self.X_ddos_t = torch.FloatTensor(X_train[mask_ddos]).to(self.device)
        ...
        logger.info(
            ...
            len(X_train), mask_ddos.sum(), len(X_test),
        )
    ...
    def pretreinar_defensor(self) -> None:
        ...
        logger.info("Pré-treinando Defensor por %d epochs...", self.cfg.epochs_pretrain)
        self.defensor.train()
        ...
        for epoch in range(self.cfg.epochs_pretrain):
            for X_batch, y_batch in self.train_loader:
                ...
                loss.backward()
                self.opt_def.step()
            logger.debug("  Pré-treino epoch %d/%d", epoch + 1, self.cfg.epochs_pretrain)

        acc = self._medir_acuracia()
        logger.info("Defensor pré-treinado — acurácia: %.2f%%", acc * 100)

    # ── Ciclo adversarial ──────────────────────────────────────────────────

    def rodar_ciclo(self) -> dict:
        """
        Executa N_RODADAS de co-evolução Atacante ↔ Defensor.
        Retorna o histórico completo.
        """
        logger.info("Iniciando ciclo adversarial (%d rodadas)", self.cfg.n_rodadas)

        for rodada in range(self.cfg.n_rodadas):
            loss_atk, taxa_evasao = self._treinar_atacante()
            loss_def = self._retreinar_defensor()
            acc = self._medir_acuracia()

            self.cfg.historico["taxa_evasao"].append(taxa_evasao)
            self.cfg.historico["acuracia_defensor"].append(acc)
            self.cfg.historico["loss_atacante"].append(loss_atk)
            self.cfg.historico["loss_defensor"].append(loss_def)

            logger.info(
                "Rodada %2d/%d | Evasão: %5.1f%% | Acc Defensor: %.2f%%",
                rodada + 1, self.cfg.n_rodadas,
                taxa_evasao * 100, acc * 100,
            )

            self._salvar_checkpoint(rodada + 1)

        return self.cfg.historico

    def _treinar_atacante(self) -> tuple[float, float]:
        """Fase 1: Atacante aprende a perturbar DDoS para enganar o Defensor."""
        # Congelar Defensor
        ...
        for epoch in range(self.cfg.epochs_por_rodada):
            for (X_ddos_batch,) in self.ddos_loader:
                ...
                if epoch == self.cfg.epochs_por_rodada - 1:
                    evasoes += (predicoes < 0.5).sum().item()
                    total += batch_size

        taxa_evasao = evasoes / max(total, 1)
        loss_media = loss_total / (self.cfg.epochs_por_rodada * len(self.ddos_loader))
    ...
    def _retreinar_defensor(self) -> float:
        """Fase 2: Defensor aprende a detectar as amostras perturbadas."""
        self.defensor.train()
        self.atacante.eval()
        ...
        with torch.no_grad():
            ...
            z = torch.randn(n, self.cfg.noise_dim).to(self.device)
            idx = torch.randperm(len(self.X_ddos_t))[:n]
            X_ddos_sample = self.X_ddos_t[idx]
            amostras_perturbadas = X_ddos_sample + self.cfg.epsilon * self.atacante(z)

        # Combinar dados reais + perturbados
        idx_real = torch.randperm(len(self.X_train_t))[:n]
        X_real = self.X_train_t[idx_real]
        y_real = self.y_train_t[idx_real]

        X_comb = torch.cat([X_real, amostras_perturbadas])
        y_comb = torch.cat([y_real, torch.ones(n).to(self.device)])
        ...
        return loss_total / (self.cfg.epochs_por_rodada * len(loader))

    def _medir_acuracia(self) -> float:
        self.defensor.eval()
        with torch.no_grad():
            pred = self.defensor(self.X_test_t)
            return ((pred >= 0.5).float() == self.y_test_t).float().mean().item()
    ...
    def carregar_checkpoint(self, path_atacante: Path, path_defensor: Optional[Path] = None) -> None:
        self.atacante.load_state_dict(torch.load(path_atacante, map_location=self.device))
        if path_defensor:
            self.defensor.load_state_dict(torch.load(path_defensor, map_location=self.device))
